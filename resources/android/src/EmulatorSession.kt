...
/**
 * One running emulator, alive until [release] or process death; views lend it
 * a Surface and leave. Its render thread is the one thread libco lets ares run on.
 */
class EmulatorSession(context: Context) {
    ...
    // Last declarative setup (system/config/rom) applied, so a remount with the
    // same props does not re-stage a running game.
    ...
    // -----------------------------------------------------------------------
    // Surface attach/detach — driven by whichever EmulatorRenderer is on screen.
    // -----------------------------------------------------------------------

    /** A game paused because its last view left; resumes when one attaches. */
    @Volatile private var pausedByDetach = false

    /** The one thread every ares call runs on; idles at 250ms ticks while there is nothing to draw. */
    private fun ensureRenderThread() {
        val t = renderThread
        if (t == null || !t.isAlive) renderThread = RenderThread().also { it.start() }
    }

    init {
        // Staging can arrive from PHP before any view attaches; ares must still run on this thread.
        ensureRenderThread()
    }
    ...
    /** On a fast remount the new view's surface is attached before the old view's destroy arrives. */
    fun detachSurface(surface: Surface) {
        if (currentSurface !== surface) return

        if (romLoaded && currentStatus == "running") {
            pausedByDetach = true
            pauseEmulation()
        }
        ...
        if (!latch.await(2, TimeUnit.SECONDS)) {
            Log.w(TAG, "detachSurface: render thread didn't release the surface in time")
        }
    }
    ...
    fun syncSetEngineOption(key: String, value: String, staged: Boolean): String? {
        ensureRenderThread()
        return syncOnGlThread { core.setEngineOption(key, value, staged) }
    }
    ...
    /**
     * Stop audio and tear down the ares core for good; views cannot reattach.
